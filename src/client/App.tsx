    ...
    try {
      if (nextLive) {
        ...
        const fresh = await fetchView(
          { ingestSequence: seq, eventTime: 0 },
          false,
        );
        setView(fresh);
      }
  ...
  return (
    <div className="app">
      <header className="header">
        ...
        {live ? (
          <span className="badge live">● 实时跟随</span>
        ) : (
          <span className="badge paused">❚❚ 已暂停回放</span>
        )}
        <span className="badge">
          ledger {view?.ledgerInfo.totalRecords ?? 0} 条
        </span>
        <span className="badge">spans {view?.spans.length ?? 0}</span>
        <span className="badge">错误路径 {view?.errorPaths.length ?? 0}</span>
        {error ? (
          <span className="badge" style={{ color: "var(--error)" }}>
            {error}
          </span>
        ) : null}
      ...
      <aside className="panel left">
        ...
        <div className="panel-body">
          ...
          <SpanList
            spans={visibleSpans}
            selectedKey={
              selected ? `${selected.traceId}:${selected.spanId}` : null
            }
      ...
      <aside className="panel right">
        <div className="panel-title with-tabs">
          ...
          <button
            className={`tab-btn ${rightTab === "snapshots" ? "active" : ""}`}
      ...
      <section className="panel timeline-panel">
        ...
        <div className="cursor-meta">
          <span>
            cursor.eventTime:{" "}
            <b>{formatClock(effectiveCursor?.eventTime ?? 0)}</b>
          </span>
          <span>
            cursor.ingestSequence: <b>{effectiveCursor?.ingestSequence ?? 0}</b>
          </span>
          <span>
            eventTime 范围:{" "}
            <b>
              {formatClock(view?.ledgerInfo.minEventTime ?? 0)} ~{" "}
              {formatClock(view?.ledgerInfo.maxEventTime ?? 0)}
            </b>
          </span>
          <span>
            视图生成: <b>{formatClock(view?.generatedAt ?? 0)}</b>
          </span>
        </div>
      </section>
    </div>
